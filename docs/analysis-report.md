...
> 核查范围：81 个后端主代码文件、4 个后端测试文件、11 个 Vue 文件、39 个 TypeScript 文件、Flyway 脚本、OpenAPI、CI、Nginx 与运行配置

## 1. 结论摘要

项目当前是结构清晰的模块化单体，导入、版本管理、阅读、离线缓存、文档下架与永久删除链路已经形成闭环。现有测试并非原报告所称的“核心服务基本无覆盖”：本次使用 JDK 21 实测后端 50 个用例全部通过，前端 14 个测试文件共 54 个用例全部通过，API 契约检查覆盖 48 个端点并通过。

本轮核查确认需要优先处理的问题主要集中在：版本号与阅读进度的并发一致性、编辑器自动保存竞态、安全基线、导入幂等语义、部分 N+1/逐行数据库操作、离线队列并发、PWA 更新策略，以及实体类公开字段。原报告中的“缺少已有索引”“50MB+ 文件无界加载”“所有异常都按 ERROR 记录”“必须引入 Caffeine/Vuex/WebSocket”等结论不完全符合当前代码，已在本报告中纠正。
...
1. 默认后端端口 `28080` 同时出现在 `application.yml`、前端代理默认值、Nginx 示例和 README。部署层确实需要分别声明端口，但当前契约脚本没有验证这些默认值一致。
2. 上传限制虽然已由契约脚本校验，但 `ApiExceptionHandler` 的“不能超过 10MB”仍是硬编码文案；调整配置时可能出现实际限制与提示不一致。
3. `storage-dir`、`converter-version` 虽可按 Spring 配置优先级由环境变量覆盖，但生产模板没有显式列出对应变量，也没有非敏感配置摘要，运维时不易确认最终值。
4. 前端导入轮询 `800ms`、离线内容上限 `30` 分散在具体模块中，属于产品策略常量，后续调优时不易发现和统一测试。

**修复计划**：

- 将 `server.port`、存储目录、转换器版本改为“环境变量 + 安全默认值”，例如 `${SERVER_PORT:28080}`；README 和部署模板列出同名配置项。
- 新增经过校验的 `UploadProperties`，异常提示从实际 `DataSize` 生成；契约脚本解析配置值而不是依赖固定字符串匹配。
- 在前端建立 `shared/runtimePolicy.ts`，只收敛轮询间隔、缓存容量等跨模块产品策略；算法内部常量无需全部外部化。
- 扩展配置契约测试：默认端口、上传限制、Profile 必填项、H2/MySQL Flyway 版本必须一致。

**验收标准**：修改任一契约值时，未同步的前端、部署模板或提示文案会使 CI 失败；生产启动日志只输出非敏感配置摘要，不输出密码。
...
**验收标准**：15 个实体 public 实例字段计数为 0；后端编译、50 个现有测试、MyBatis-Flex CRUD/大字段映射及真实 MySQL 冒烟测试全部通过。
...
| 检查 | 结果 |
|---|---|
| 后端测试（JDK 21） | 50 passed |
| 前端 Vitest | 14 files / 54 passed |
| API/配置契约 | 48 endpoints，passed |
| CI | 已执行前端测试/构建、后端测试/打包；暂无 E2E、静态规则和真实 MySQL 测试 |
...
1. **真实 MySQL 集成测试**：Flyway 迁移、版本号并发、进度 upsert、批量删除、索引与关键 SQL 方言。
2. **组件测试**：ReaderView 离线错误分类；VersionEditor 自动保存、节点切换与旧请求返回。
3. **浏览器 E2E**：登录、移动端 `≤760px` 禁止后台、桌面端管理、导入到发布、下架、彻底删除、PWA 升级。
4. **安全测试**：登录限流、CSRF/Origin、Cookie 属性、安全头、actuator 路径白名单。
5. **架构规则**：实体无 public 实例字段；Controller/DTO 契约不绕过 OpenAPI；生产 Profile 不允许弱默认凭据或同步导入。
6. **性能基准**：大文档导入、目录加载、版本列表和永久删除的 SQL 数与耗时预算。
...
| B：实体与可读性 | 15 个实体私有字段、Lombok Getter/Setter、迁移直接字段访问、关键逻辑中文注释 | public 实例字段为 0，现有 104 个前后端测试全部通过 |
