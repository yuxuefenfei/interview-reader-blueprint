...
export const adminApi = {
  ...
  updateNode: (versionId: string, nodeId: string, draftRevision: number, node: Pick<EditorSnapshot["nodes"][number], "title" | "nodeType" | "semanticRole">) => http.patch<EditorSnapshot>(`/admin/versions/${versionId}/editor/nodes/${nodeId}`, { draftRevision, ...node }).then(({ data }) => data),
  deleteNode: (versionId: string, nodeId: string, draftRevision: number) => http.delete<EditorSnapshot>(`/admin/versions/${versionId}/editor/nodes/${nodeId}`, { params: { draftRevision } }).then(({ data }) => data),
