...
<template>
  <section class="admin-view import-view">
    <AdminPageHeader eyebrow="内容入库" title="导入中心" description="上传后自动识别文件类型，经过转换、校验后生成可编辑的版本草稿。">
      <template #status><el-tag v-if="job" effect="plain">{{ zh(job.status) }}</el-tag></template>
    </AdminPageHeader>
    <div class="import-layout">
      ...
      <el-card shadow="never" class="import-progress-card">
        ...
        <template v-else>
          ...
          <div class="job-actions ui-action-row"><el-button v-if="job.status === 'READY'" type="success" :icon="CircleCheck" :loading="committing" :disabled="!preview" data-testid="commit-import" @click="commit">生成可编辑草稿</el-button><el-button v-if="job.status === 'REVIEW_REQUIRED'" type="warning" disabled>请处理校验问题后重试</el-button></div>
        </template>
      </el-card>
    </div>
  </section>
